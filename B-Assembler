...
def l_type(rd, rs1, rs2, register):
    if rd not in register or rs1 not in register or rs2 not in register:
        return None, "Invalid register"
    regd = register[rd]
    reg1 = register[rs1]
    reg2 = register[rs2]
    binary = '1111111' + reg2 + reg1 + '111' + regd + op
    return binary, None
...
if ['beq', 'zero,zero,0\n'] not in lines and ['beq', 'zero,zero,0'] not in lines:
    ...
    for i in lines:
        if len(i) != 2:
            ...
            if len(c) != 3:
                valid_instructions = False
                print("Invalid number of arguments for l type")
                break
            else:
                binary, error = l_type(c[0], c[1], c[2], register)
                if error:
                    valid_instructions = False
                    print(error)
                    break
                    ...
                    valid_instructions = False
                    print("Invalid register for l type")
                    break
        elif i[0] in mtype:
            c=i[1].split(',')
            if len(c)!=2:
                valid_instructions=False
                print("Invalid number of arguments for m type")
                break
            else:
                m_type(c)
        else:
